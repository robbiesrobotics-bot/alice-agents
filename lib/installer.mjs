...
import { getHermesConfigPath, getHermesProfileDir } from './hermes-paths.mjs';
...
async function detectRuntime(...preferredRuntimes) {
  ...
  const hermesDir = getHermesProfileDir();
  const hermesConfig = getHermesConfigPath();
...
export async function runInstall(options = {}) {
  ...
  if (runtime === 'hermes') {
    ...
    console.log(`  ${dim('Skills dir:')}      ${cyan(getHermesProfileDir() + '/skills/alice/')}`);
    console.log('');
    printSeparator();
    console.log('');
    return;
