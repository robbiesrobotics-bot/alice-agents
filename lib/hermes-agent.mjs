...
import { getHermesBaseDir, getHermesProfileDir, getHermesSkillsDir } from './hermes-paths.mjs';

const __dirname = dirname(fileURLToPath(import.meta.url));
const HERMES_DIR = getHermesProfileDir();
const HERMES_BASE_DIR = getHermesBaseDir();
const OPENCLAW_DIR = join(homedir(), '.openclaw');
const SKILLS_PARENT = getHermesSkillsDir(); // skills live under the active Hermes profile
...
function findHermesBinary() {
  const candidates = [
    ...
    join(HERMES_BASE_DIR, 'bin', 'hermes'),
...
export class HermesAgentManager {
   ...
   *
   * Creates:
   *   - active Hermes profile skills/<category>/<agentId>/SKILL.md
